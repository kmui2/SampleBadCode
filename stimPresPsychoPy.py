...
def calculateRectangularCoordinates(distanceX, distanceY, numCols, numRows, yOffset=0, xOffset=0):
    ...
    for curCol in range(0, numCols):  # x-coord
        for curRow in range(0, numRows):  # y-coord
            ...
            curObj = curObj + 1
    xCorrect = max(map(lambda x: x[0], coord)) / 2 - xOffset
    yCorrect = max(map(lambda x: x[1], coord)) / 2 - yOffset
...
def playAndWait(sound, soundPath='', winSound=False, waitFor=-1):
    ...
    if winSound:
        ...
        if waitFor != 0:
            ...
            if soundPath:
                winsound.PlaySound(
                    soundPath, winsound.SND_FILENAME | winsound.SND_ASYNC)
            else:
...
def showText(win, textToShow, color=[-1, -1, -1], waitForKey=True, acceptOnly=0, inputDevice="keyboard", mouse=False, pos=[0, 0], scale=1):
    ...
    textStim = visual.TextStim(
        win, pos=pos, wrapWidth=wrapWidth, color=color, height=height, text=textToShow)
    textStim.draw()
    win.flip()
    if mouse:
...
def waitingAnimation(win, size=20, distanceBetweenElements=3, numElements=8, delay=1.0):
    ...
    for curFrame in range(numElements, -1, -1):
        for curElement in range(curFrame):
            visual.PatchStim(win, color='white', size=size, tex='None', mask='circle', pos=[
                             positions[curElement], 0]).draw()
        win.flip()
        core.wait(delay)
